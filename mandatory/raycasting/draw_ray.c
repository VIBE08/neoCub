...
/*   Updated: 2024/10/31 17:52:53 by ael-hara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_vertical(t_vintersection *inter, int x, int y, double angle)
{
	...
	inter->next_vertical_x = floor(x / GRID_SIZE) * GRID_SIZE;
	if (inter->facing_right)
		inter->next_vertical_x += GRID_SIZE;
	inter->next_vertical_y = y + (inter->next_vertical_x - x)
		* tan(inter->angle);
...
void	init_horizontal(t_intersection *inter, int x, int y, double angle)
{
	...
	inter->facing_right = inter->angle < 0.5 * PI || inter->angle > 1.5 * PI;
	inter->facing_left = !inter->facing_right;
	inter->next_horizontal_y = floor(y / GRID_SIZE) * GRID_SIZE;
	if (inter->facing_down)
		inter->next_horizontal_y += GRID_SIZE;
	inter->next_horizontal_x = x + (inter->next_horizontal_y - y)
		/ tan(inter->angle);
...
double	horizontal_intersection(t_map_data *map, int x, int y, double angle)
{
	...
	return (map->wall_hit_y_h = map->wall_hit_x_h, map->wall_hit_x_h);
}
...
void	min_intersection(t_map_data *map, int x, int y, double angle)
{
	...
	if (map->player->distance > MAX_DISTANCE * GRID_SIZE)
		map->player->distance = MAX_DISTANCE * GRID_SIZE;
}
