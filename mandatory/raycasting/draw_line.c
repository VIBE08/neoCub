...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/31 21:12:29 by ael-hara          #+#    #+#             */
/*   Updated: 2024/11/26 12:28:29 by ael-hara         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	get_xcord(t_map_data *map, mlx_texture_t *texture)
{
	if (map->vertical == 1)
	{
...
mlx_texture_t	*choose_texture(t_map_data *map, double angle)
{
	...
	if (map->vertical)
	{
		if (angle > 1.5 * PI || angle < 0.5 * PI)
			texture = map->textures->texture_e;
		else
			texture = map->textures->texture_w;
	}
	else
	{
		if (angle > 0 && angle < PI)
			texture = map->textures->texture_n;
		else
			texture = map->textures->texture_s;
	}
	return (texture);
}
